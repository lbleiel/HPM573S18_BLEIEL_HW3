...
class ChanceNode(Node):
    ...
    def get_health_utility(self):
        ...
        """
        exp_health = self.health  # expected health initialized with the health of the current node
        i = 0
        for node in self.futureNodes:
            exp_health += self.probs[i]*node.get_health_utility()
            i += 1
        return exp_health
...
print(C1.get_expected_cost()) # 31.6
print(C1.get_health_utility()) # 0.744

print(C3.get_expected_cost()) # 50.0
print(C3.get_health_utility()) # 0.52
